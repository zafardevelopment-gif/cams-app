...
export default async function SuperAdminPage() {
  ...
  const [{ data: hospitals }, { data: registrations }, { data: users }] = await Promise.all([
    ...
    admin.from(T.users).select('id, status').eq('status', 'active'),
  ])
    ...
    <>
      ...
      <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
        <div className="kpi-card">
          ...
          <div className="kpi-value">{activeHospitals.length}</div>
        </div>
        <div className="kpi-card">
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value" style={{ fontSize: 18 }}>Active</div>
          <div className="kpi-change up">All systems normal</div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="card-body p-0">
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>Name</th><th>Email</th><th>Date</th><th></th></tr>
                </thead>
                <tbody>
                  {(registrations ?? []).map((r) => (
                    <tr key={r.id}>
                  ...
                  {(registrations ?? []).length === 0 && (
                    <tr><td colSpan={4} style={{ textAlign: 'center', color: 'var(--gray-400)', padding: 24 }}>No pending registrations</td></tr>
                  )}
                </tbody>
              </table>
            </div>
